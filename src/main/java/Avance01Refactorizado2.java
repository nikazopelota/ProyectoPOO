...
public class Avance01Refactorizado2 {
        ...
        switch (opcion) {
            ...
            case 2:
                if (registrarUsuario()) {
                    ...
                    // Aquí lo devuelves al menú inicial sin iniciar sesión directamente
                }
                break;
            case 3:
    ...
    public static boolean iniciarSesion() {
        ...
        while (true) {
            ...
            if (matriculaActual.equals("0")) {
                // Volver al menú inicial si ingresa "0"
                return false;
            }

            if (usuarios.contains(matriculaActual)) {
                System.out.println("Bienvenido!");
                return true; // Inicia sesión exitosamente
            } else {
    ...
    private static void mostrarDisponibilidadLogias(List<Integer> logias, String mensaje, int capacidadActual) {
        System.out.print(mensaje + " ");
        logias.stream()
    ...
    public static void reservarLogia() {
        if (reservas.containsKey(matriculaActual)) {
            System.out.println("Ya tienes una logia reservada: Logia " + reservas.get(matriculaActual).split(":")[0] + ".");
            return;
        }
    ...
    private static List<Integer> seleccionarLogias() {
        ...
        return seleccionarLogiasPorOpcion(opcion);
    }
    ...
    public static void cancelarReserva() {
        if (!reservas.containsKey(matriculaActual)) {
            ...
            // Verificar si la logia ya está presente en la lista antes de agregarla
            if (capacidad == 7 && !logias7Personas.contains(numeroLogia)) {
                logias7Personas.add(numeroLogia);
            } else if (capacidad == 5 && !logias5Personas.contains(numeroLogia)) {
                logias5Personas.add(numeroLogia);
            } else if (capacidad == 3 && !logias3Personas.contains(numeroLogia)) {
                logias3Personas.add(numeroLogia);
            }
